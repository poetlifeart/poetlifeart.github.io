...
Not every equation gives rise to a recursive solution obviously and here \\(d_\beta(s')\\) inverse can be a large number. So why does dropping it work? "Who knows is the right answer".  C. Gelada, M. Bellemare (2019) show it is meaningful, has nice properties and works in practice. A rough idea might be that from the point of sample paths, we could drop   \\(d_\beta(s')\\) if we could draw according to it. We are; however, already drawing  \\(s'\\) conditionally correctly and can assume we are also drawing it according to \\(d_\beta(s')\\) after some iterations, so it was sort of there and go cancelled by dividing by \\(d_\beta(s')\\) but we OK since the conditional is still there. 